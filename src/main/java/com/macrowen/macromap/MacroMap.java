		...
		class Floor {
			...
			void changeHighlight(float x, float y) {
				...
				if (unit != null) {
					...
					if (mShop != null
						...
						x = Math.min(x, getWidth() - mShopPosition.getWidth()
								/ 2);
						...
						if (mShop.mRegion.contains((int) x, (int) y)) {
							...
							params.leftMargin = (int) x
									- mShopPosition.getWidth() / 2;
							params.topMargin = (int) y
									- mShopPosition.getHeight();
			...
			void draw(Canvas canvas) {
				...
				if (mShop != null
					...
					if (mShop.mRegion.contains((int) x, (int) y)) {
						RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) mShopPosition
								.getLayoutParams();
						params.leftMargin = (int) x - mShopPosition.getWidth()
								/ 2;
			...
			void setOffset(float x, float y) {
				...
				if (mBorder.width() * mScale <= getWidth()) {
					...
					// getWidth() / mScale * margin);
					x = Math.min(x, -mBorder.left + getWidth() / 2 - mMapMargin
							/ mScale);
					x = Math.max(x, -mBorder.right + getWidth() / 2
							+ mMapMargin / mScale);
				}
				if (mBorder.height() * mScale <= getHeight()) {
					...
					// getHeight() / mScale * margin);
					y = Math.min(y, -mBorder.top + getHeight() / 2 - mMapMargin
							/ mScale);
					y = Math.max(y, -mBorder.bottom + getHeight() / 2
							+ mMapMargin / mScale);
				}
			...
			void setScale(float scale) {
				...
				scale = Math.max(scale, Math.min((getWidth() - mMapMargin)
						/ mBorder.width(),
